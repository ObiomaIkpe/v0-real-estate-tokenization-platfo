...
import { useRWAContract, CreateAssetParams } from "@/lib/hooks/useRWAContract";
import { parseUnits, Hex } from "viem";
import { toast } from "sonner"; // Assuming you use a toast/notification library
// --------------------------

// Define the decimal for your asset's price.
...
export default function PropertyReviewPage() {
  ...
  const { createAsset } = useRWAContract();

  // Monitor the transaction status for confirmation
  const {
    data: receipt,
  ...
  // 💡 FIX: Define fetchProperty outside of useEffect for universal access
  ...
  }, [propertyId]); // Dependency array ensures it only updates if propertyId changes
  ...
  }, [propertyId, fetchProperty]); // fetchProperty is now a stable dependency

  // ----------------------------------------------------------------------
  // 2. TRANSACTION CONFIRMATION EFFECT
  useEffect(() => {
    if (isConfirmed && txHash) {
      toast.success(`Tokenization confirmed! Block: ${receipt?.blockNumber}`);

      // Re-fetch property data to update the UI status from the database
      setTimeout(() => {
        fetchProperty();
        setTxHash(undefined);
        setActionLoading(null);
    ...
    if (isTxError && txHash) {
      toast.error(
        "Transaction failed or reverted on chain. Check explorer for details."
      );
      setTxHash(undefined);
      setActionLoading(null);
    }
  }, [isConfirmed, isTxError, txHash, receipt, fetchProperty]);
  // ----------------------------------------------------------------------
  ...
  const handleApprove = async () => {
    try {
      ...
      // Update property data after successful backend action
      const updatedData = await adminService.getPropertyDetails(propertyId);
      setProperty(updatedData);
  ...
  const handleTokenize = async () => {
    if (!property || !property.pinataUrls.metadata) {
      toast.error("Property data or metadata URI is missing. Cannot tokenize.");
      return;
    }
    if (!isConnected) {
      toast.warning("Please connect your wallet to submit the transaction.");
      return;
    }
    if (isConfirming) {
      toast.info("A transaction is already pending confirmation.");
      return;
    }
    ...
    try {
      ...
      // 1. Convert value to BigInt in the smallest unit
      const requestedValueString = String(property.requestedValue);
      const priceInSmallestUnit = parseUnits(
        requestedValueString,
        ASSET_DECIMALS
      );

      // 2. Prepare payload
      const payload: CreateAssetParams = {
        name: property.propertyTitle,
        tokenURI: property.pinataUrls.metadata,
        imageURL: property.pinataUrls.images?.[0] || "",
        price: priceInSmallestUnit,
        description: property.description,
      };

      // 3. Call the smart contract via the custom hook
      const hash = await createAsset(payload);
      setTxHash(hash);

      toast.info(
        `Transaction sent. Hash: ${hash.slice(0, 10)}... Please approve in your wallet and wait for confirmation.`
      );
    } catch (error) {
      console.error("Error submitting tokenization transaction:", error);
      toast.error(
        "Tokenization failed. Ensure you have enough gas and permissions."
      );
    } finally {
      // If the submission fails before getting a hash, reset loading
      if (!txHash) setActionLoading(null);
    }
  };
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                {property.status === "approved" && (
                  <Button
                    onClick={handleTokenize}
                    disabled={!isConnected || actionLoading === "tokenize"}
                    className="w-full bg-blue-600 hover:bg-blue-700"
                  >
                    {actionLoading === "tokenize" ? (
                      <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                ...
                {txHash && (
                  <div className="text-center p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                    {isConfirming && (
                      <Loader2 className="h-6 w-6 text-yellow-600 mx-auto mb-2 animate-spin" />
                    )}
                    {isTxError && (
                    ...
                    {receipt?.blockNumber && (
                      <p className="text-xs text-gray-500">
                        Block: {Number(receipt.blockNumber).toLocaleString()}
                      </p>
                    )}
                  </div>
                )}
